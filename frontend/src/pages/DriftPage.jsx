...
export default function DriftPage() {
  ...
  const handleDeleteDrift = async (detectionResultId) => {
    if (!detectionResultId) {
      console.error("Missing detection result ID");
      return;
    }

    const confirmed = window.confirm(
      "Delete this drift result? This cannot be undone."
    );
    ...
    try {
      setDeletingId(detectionResultId);

      await api.deleteDrift(detectionResultId);

      setDrift((current) =>
        current.filter(
          (item) => item.detection_result_id !== detectionResultId
        )
      );
    } catch (error) {
      window.alert(`Failed to delete drift: ${error.message}`);
    } finally {
      setDeletingId(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
            ...
            {drift.map((d) => (
              ...
              >
                ...
                <Button
                  variant="secondary"
                  onClick={() =>
                    handleDeleteDrift(d.detection_result_id)
                  }
                  disabled={deletingId === d.detection_result_id}
                >
                  <Trash2 size={14} />
                  {deletingId === d.detection_result_id
                    ? "Deleting..."
                    : "Delete"}
                </Button>
              </li>
      ...
      <Panel
        ...
        eyebrow="Wazuh inventory snapshots"
      >
        {!configurationDrift ||
        configurationDrift.status === "insufficient_history" ? (
          <EmptyState
            title="Need two Wazuh sync snapshots"
            hint="Run Environment sync twice to compare rule configuration."
          />
          ...
          <ul className="divide-y divide-bg-800">
            {configurationDrift.changes.map((change) => (
              <li
                key={`${change.rule_id}-${change.category}`}
      ...
      >
        <div className="space-y-4">
          ...
          {productionLoading ? (
            ...
            <ErrorNote
              message={
                "Wazuh manager unreachable — check WAZUH_BASE_URL and network connectivity."
              }
            />
          ) : (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {[
                ...
                <div
                  key={label}
                  className="rounded-md border border-bg-800 bg-bg-900 p-3"
                >
              ...
              <details className="rounded-md border border-bg-800 p-3">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {productionDrift.twin_only.map((techniqueId) => (
                    <Badge key={techniqueId} tone="amber">
                      {techniqueId}
                    </Badge>
                  ))}
                </div>
